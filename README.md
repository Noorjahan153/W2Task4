...
* sudo curl -L "https://github.com/docker/compose/releases/latest/download/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose->Installing
...
By using Docker networks, containers can:

* Talk to other containers
* Access the internet
* Expose services to users

Docker automatically creates networks, and users can also create custom networks.

**Types of networks**
- bridge -Bridge is Docker’s default network .Containers connected to the same bridge network can communicate with each other.Mostly used for applications running on a single host.
- host -Host network removes network isolation between container and host.The container uses the host machine’s IP and ports directly.Provides high performance but less security.
